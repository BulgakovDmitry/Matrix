...
template <std::floating_point T> class Matrix final : public IMatrix<T> {
  private:
  ...
  public:
    ...
    Matrix(std::size_t size)
        : n_rows_{size}, n_columns_{size},
          data_{std::make_unique<Buffer<T>>(n_rows_, n_columns_)} {}

    Matrix(const Matrix &rhs)
        : n_rows_(rhs.n_rows_), n_columns_(rhs.n_columns_),
    ...
    Matrix(Matrix &&rhs) noexcept { swap(rhs); }
    ...
    [[nodiscard]] std::size_t get_n_columns() const noexcept {
        return n_columns_;
    }
    ...
    void insert(std::size_t i, std::size_t j, const T &value) {
        if (i >= n_rows_ || j >= n_columns_) {
            throw std::runtime_error(
                "the insertion area exceeds the size of the buffer\n");
        }
        (*this)[i][j] = value;
    }
  ...
  private:
    ...
    void swap_rows(std::size_t row_1, std::size_t row_2) {
        ...
        for (std::size_t j = 0; j < n_columns_; ++j) {
            std::swap((*this)[row_1][j], (*this)[row_2][j]);
        }
    }
    ...
    void swap_columns(std::size_t col_1, std::size_t col_2) {
        ...
        for (std::size_t i = 0; i < n_rows_; ++i) {
            std::swap((*this)[i][col_1], (*this)[i][col_2]);
        }
    }

    std::size_t
    find_elem_with_max_modulus_in_column(std::size_t column,
        ...
        if (start_row >= n_rows_) {
            throw std::out_of_range("start_row out of range");
        }
    ...
    Matrix<T> &
    convert_matrix_to_up_triang_form(bool *even_num_of_perms_flag = nullptr) {
